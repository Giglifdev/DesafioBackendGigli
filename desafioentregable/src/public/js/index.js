...
form.addEventListener("submit", async (evt) => {
  ...
  try {
    ...
    if (result.status === 400) return "Fill in the fields";
  } catch (error) {
...
socket.on("refreshProducts", (data) => {
  ...
  data.forEach((product) => {
    ...
    <section style="padding: 1.5rem">
      ...
      <button class="delete-button" id=${product.id} style="background-color: #231942; padding:5px 10px; border-radius:25px; cursor:pointer; font-weight:bold">Eliminar</button>
    </section>
    `;
  });
});
